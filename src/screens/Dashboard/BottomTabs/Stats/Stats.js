...
const Stats = (props) => {
  ...
  const getUserType = async () => {
    ...
    const upgradepopup = await getDataFromLocalStorage("upgradepopup")

    if (upgradepopup == "showpopup") {
      if (JSON.parse(temp) == "1") {
        //Alert.alert("Alert",strings.upgradenow)
        Alert.alert('Alert', strings.upgradenow, [
          {
            text: 'Cancel',
          ...
          { text: 'OKAY', onPress: () => { props.navigation.navigate('Upgrade') } },
